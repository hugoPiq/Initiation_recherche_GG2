...
def generate_launch_description():
    ...
    urdf = os.path.join(get_package_share_directory(
        'my_robot_description'), 'urdf/', urdfName + '.urdf')
    ...
    ld = LaunchDescription([
        ...
        Node(package='robot_state_publisher',
             node_executable='robot_state_publisher', output='screen', arguments=[urdf]),
        Node(package='my_robot_utils_scripts', node_executable='spawn_my_robot.py',
             arguments=[urdf], output='screen'),
        Node(package='my_robot_control', node_executable='control.py',
             output='screen'),
        # Node(package='hros_cognition_mara_components', node_executable='hros_cognition_mara_components',
        #      output='screen', arguments=["-motors", install_dir + configFile, "sim"])
    ])
    return ld
